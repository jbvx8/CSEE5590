...
var url='mongodb://root:password1@ds115971.mlab.com:15971/csee5590';//1.Modify this url with the credentials of your db name and password.
...
var deleteDocument = function(db, data, callback) {
    db.collection("books").deleteOne({_id: new mongodb.ObjectID(data)}, function(err, result) {
        if(err) {
            res.write("Unable to delete.");
            res.end();
        }
        console.log(data);
        console.log("Deleted the book from the collection.");
        callback();
    })
};
